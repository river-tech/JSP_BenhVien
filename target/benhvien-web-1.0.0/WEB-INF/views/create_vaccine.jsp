...
<div class="card">
    ...
    <form method="post" action="${pageContext.request.contextPath}/vaccines/create">
        ...
        <div class="form-group">
            <label for="maBenh">Các bệnh có thể phòng được *</label>
            <select id="maBenh" name="maBenh" multiple size="5" required style="height: 120px;">
                <c:forEach var="b" items="${diseases}">
                    <option value="${b.maBenh}">${b.maBenh} - ${b.tenBenh}</option>
                </c:forEach>
            </select>
            <small style="color: #666;">(Giữ Ctrl/Cmd để chọn nhiều bệnh)</small>
        </div>
